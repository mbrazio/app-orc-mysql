...
**Versão:** 1.1.0  
**Stack de Tecnologias:** Next.js 16 (App Router), React 19, MySQL (mysql2/promise + Hostinger), Auth (JWT via jose + bcryptjs), Tailwind CSS v4, Decimal.js, React Hook Form, Zod, Vitest.

---

## 🎯 1. Objetivos da Suíte de Testes

Garantir a estabilidade, segurança e integridade operacional do aplicativo **FinanceFlow**, cobrindo:
1. **Integridade Matemática Financeira:** Precisão decimal sem erros de ponto flutuante em todos os cálculos monetários via `Decimal.js`.
2. **Segurança e Privacidade:** Criptografia de senhas com `bcryptjs`, sessões em cookies HTTP-Only com `jose` (JWT), tratamento amigável de exceções de autenticação e proteção contra inserções inválidas.
...
| **Mocks de API** | **MSW (Mock Service Worker)** | Simulação de respostas da API REST (`/api/*`) e tratamento de respostas HTTP 401/403 |
...
- **Arquivos-Chave:** `app/(auth)/login/page.tsx`, `app/(auth)/cadastro/page.tsx`, `proxy.ts`, `lib/auth.ts`, `app/api/auth/*`
...
- **Arquivo-Chave:** `app/(dashboard)/transactions/page.tsx`, `app/api/transactions/*`
...
| **TX-03** | Soft Delete de Transação | Clique em Excluir + Confirmação | Disparar mutação DELETE para `/api/transactions/[id]` que grava `deleted_at: CURRENT_TIMESTAMP` |
...
- **Arquivo-Chave:** `app/(dashboard)/categories/page.tsx`, `app/api/categories/*`
...
| **CAT-04** | Soft Delete de Categoria | Exclusão de categoria customizada | Atualizar `deleted_at` no MySQL preservando o histórico de transações associadas |

---

### 3.4. Módulo de Metas Financeiras (Goals)
- **Arquivo-Chave:** `app/(dashboard)/goals/page.tsx`, `app/api/goals/*`
...
- [x] Rotas protegidas exigem autenticação ativa via JWT em cookies HTTP-Only no middleware (`proxy.ts`).
